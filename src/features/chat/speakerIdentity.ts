/**
 * 说话人身份解析（2026-09-16 消息卡「角色色轨卡」拍板的配套逻辑）：消息卡左缘
 * 3px 色轨的颜色即说话人身份——角色条取其模板卡强调色（accentColorOf：显式
...
/** 用户条色轨：中性灰（非角色即无角色色）。 */
export const USER_SPEAKER_RAIL = tokens.colorNeutralForeground3;

/** 角色条色轨：模板卡强调色派生；动态造人（无模板卡，D6）按实例溯源 id 走
 *  调色板回退，同人不同处配色漂移不可接受（posterGradient 取模纪律）。 */
export function instanceRailColor(
...
/** 消息条目色轨：user 恒中性灰；assistant 按说话人实例（characterId = 实例 id
 *  真值）解析；实例查不到（会话阵容回显缺失）诚实落中性灰，不伪装角色色。 */
export function messageRailColor(
  message: ChatMessage,
  roster: readonly SessionInstanceDto[],
  cards: readonly CharacterSummary[],
): string {
  if (message.role === 'user' || message.characterId === null) return USER_SPEAKER_RAIL;
  const instance = roster.find((i) => i.id === message.characterId);
  return instance !== undefined ? instanceRailColor(instance, cards) : USER_SPEAKER_RAIL;
}
