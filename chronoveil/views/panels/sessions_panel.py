...
class SessionsPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._add_session_button = PrimaryPushButton(FluentIcon.EDIT, "New Session")
        self._add_session_button.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
    ...
    def on_new_session_button_clicked(self):
        card = SessionCard(title="New Session", description=f"Generated at {datetime.now().strftime('%H:%M:%S')}",
