...
class LLMSettingsPanel(GroupHeaderCardWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self._setup_provider_setting()
        self._setup_base_url_setting()
        self._setup_model_name_setting()
        self._setup_api_key_setting()
        self._setup_test_setting()

    def _setup_provider_setting(self) -> None:
        self._provider_setting = ComboBox(self)
        self._provider_setting.setMinimumWidth(150)
        for llm_format in LLMProvider:
            self._provider_setting.addItem(text=llm_format.label, userData=llm_format)

        self._provider_setting_card = self.addGroup(
            icon=FluentIcon.APPLICATION,
            title=self.tr("Provider"),
            content=self.tr("Request format used to communicate with the LLM backend"),
            widget=self._provider_setting,
        )
    ...
    def _setup_connections(self) -> None:
        self._provider_setting.currentIndexChanged.connect(self.on_llm_settings_changed)
    ...
    def set_llm_api_settings(self, llm_settings: LLMSettings) -> None:
        if llm_settings.provider is not None:
            self._provider_setting.setCurrentIndex(self._provider_setting.findData(llm_settings.provider))
    ...
    def on_llm_settings_changed(self):
        ...
        llm_settings = LLMSettings(
            provider=self._provider_setting.currentData(),
    ...
    def on_language_changed(self):
        ...
        self._provider_setting_card.setTitle(self.tr("Provider"))
        self._provider_setting_card.setContent(self.tr("Request format used to communicate with the LLM backend"))
