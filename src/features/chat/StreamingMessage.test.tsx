...
function messageTree(state: StreamState, onSettled: () => void): ReactElement {
  return (
    <FluentProvider theme={webLightTheme}>
      ...
      <StreamingMessage state={state} speaker={SPEAKER} speakerColor="#888888" tuning={{ msPerChar: 10 }} onSettled={onSettled} />
    </FluentProvider>
  );
}
...
describe('tuning 中途热更（TASK-12 / 审计问题 8）', () => {
  ...
  function treeWithTuning(state: StreamState, tuning: RendererTuning, onSettled: () => void): ReactElement {
    return (
      <FluentProvider theme={webLightTheme}>
        <StreamingMessage state={state} speaker={SPEAKER} speakerColor="#888888" tuning={tuning} onSettled={onSettled} />
      </FluentProvider>
    );
  }
