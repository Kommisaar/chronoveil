...
export const en = {
  ...
  characters: {
    ...
    newSessionFailed: 'Failed to create a session',
  ...
  settings: {
    ...
    providerEmpty: 'No model services yet. Create one (OpenAI-compatible: Base URL + API Key + model name) and save to start chatting.',
    addProvider: 'Add',
    providerName: 'Name',
    ...
    deleteConfirmBody: 'Delete "{{name}}"? Changes land on disk when you click Save.',
    ...
    cancel: 'Cancel',
    hint: 'Settings are stored in ~/.chronoveil/config.json (ADR-012); clicking Save persists them and applies immediately. External edits to that file while running are overwritten by the next save. API Keys are stored in plain text on this machine (OQ-001).',
  },
} as const;
