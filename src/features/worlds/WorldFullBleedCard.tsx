...
const useStyles = makeStyles({
  ...
  // 不承载文字但满幅覆盖文字带（同层绘制在 content 之上）——静息 opacity 0
  // 无影响；hover 态 5% 白薄纱会同时提亮文字像素与实底，按守卫「纯白底
  // 未来口径」推算 meta 文字 ≈4.19:1 < 4.5（静息 4.65 恰过线）——当前世界
  // 色板恒深（worldGradientOf 无纯白路径）不可达该口径；色板将来引入浅色/
  // 用户可选色时，须把薄纱并入守卫数学或调薄，否则 hover 态跌破正文线。
  // DOM 上置于内容之后——同为定位元素
