...
// （worldbook）的卡面摘要行使用。剥离规则与渲染引擎的语法子集对齐（语义对照
// src/engine/parser.ts 的 StreamParser，含文件头的支持清单），场景线正则直接
// import 自引擎（单一事实源）：
// - 强调 `*斜体*` / `**加粗**`：闭合对剥标记留正文；未闭合的星号按字面保留
//   （引擎同款回退：段落封存/流结束未闭合按字面吐出，ADR-008）；
// - 场景线 `---` / `===` / `——`（引擎导出的 SCENE_LINE_RE，整「空行分段块」
...
import { SCENE_LINE_RE } from '../engine';
...
export function excerptOf(text: string, maxChars: number): string {
  const plain = text
    ...
    .replace(/^[ \t]*#{1,6}[ \t]+/gm, '')
    // 列表标记（行首成立；剥除范围是引擎的超集（容缩进）——正则容 [ \t]* 前缀
    // 而引擎 lineStart 不容忍前导空白（缩进标记引擎按字面渲染），方向对摘录
    // 有利；无序 `- ` 后随 -/= 是场景线形态不剥，见文件头）
    .replace(/^[ \t]*-(?![ \t]*[-=])[ \t]+/gm, '')
    .replace(/^[ \t]*\d+\.[ \t]+/gm, '')
    // 强调闭合对剥标记（先双星后单星；内容限定无星）：未闭合按字面保留
    // （对齐 ADR-008 回退）；混合嵌套是近似处置，可能部分剥除（如 `**a*b**`
    // 引擎输出 a*b，此处输出 *ab**）
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    .replace(/\*([^*]+)\*/g, '$1')
