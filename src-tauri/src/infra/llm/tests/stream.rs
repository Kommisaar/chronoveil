//! 流式生成路径（验收 2–6）：正常流、内联 think 半标签跨包、断流整条重发、
//! 取消中断，以及错误映射（验收 8：401 / 429 / 5xx / 读超时 / 连接失败）。
...
use super::*;
use crate::infra::llm::mock::{delta_json, sse_data, sse_head, status_head, MockRequest, MockServer};
...
async fn cancel_stops_stream_and_emits_nothing_more() {
    ...
    let task = tokio::spawn(async move { c.chat_stream(&messages(), IDS, sink_tx, &cancel, None).await });
